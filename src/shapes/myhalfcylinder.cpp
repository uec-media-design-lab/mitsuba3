...
#include <drjit/packet.h>
    ...
    #include "optix/myhalfcylinder.cuh"
#endif
...
class MyHalfCylinder final : public Shape<Float, Spectrum> {
public:
    ...
    MyHalfCylinder(const Properties &props) : Base(props) {
    ...
    ray_intersect_preliminary_impl(const Ray3fP &ray_,
        ...
        active &= solution_found && !out_bounds && !in_bounds &&
                  ((z_pos_near >= Value(0.0) && z_pos_near <= length && near_t >= Value(0.0) && x_pos_near >= Value(0.0)) ||
                   (z_pos_far  >= Value(0.0) && z_pos_far <= length  && far_t <= maxt && x_pos_far >= Value(0.0)));
        /* 変更ここまで */
    ...
    dr::mask_t<FloatP> ray_test_impl(const Ray3fP &ray_,
        ...
        // レイとの交点のx座標が負（左側）なら、衝突無しとする条件を追加
    ...
    void optix_prepare_geometry() override {
        if constexpr (dr::is_cuda_v<Float>) {
            if (!m_optix_data_ptr)
                m_optix_data_ptr = jit_malloc(AllocType::Device, sizeof(OptixHalfCylinderData));

            OptixHalfCylinderData data = { bbox(), m_to_object.scalar(),
                                       (float) 1.f,
                                       (float) 1.f };

            jit_memcpy(JitBackend::CUDA, m_optix_data_ptr, &data, sizeof(OptixHalfCylinderData));
        }
    }
#endif
...
MI_IMPLEMENT_CLASS_VARIANT(MyHalfCylinder, Shape)
MI_EXPORT_PLUGIN(MyHalfCylinder, "MyHalfCylinder intersection primitive");
