...
#include <mitsuba/render/shape.h>
    ...
    #include "optix/myhalfsphere.cuh"
#endif
...
class MyHalfSphere final : public Shape<Float, Spectrum> {
public:
    ...
    MyHalfSphere(const Properties &props) : Base(props) {
    ...
    ray_intersect_preliminary_impl(const Ray3fP &ray,
        ...
        Ray3fP ray_local;
        if constexpr (!dr::is_jit_v<Value>)
            ray_local = m_to_object.scalar().transform_affine(ray);
        else
            ray_local = m_to_object.value().transform_affine(ray);
        /* ここまで追加 */
        ...
        /* 以下追加・変更 */
        Value oz = Value(ray_local.o.z()),
              dz = Value(ray_local.d.z());
        Value z_pos_near = oz + dz * near_t,
              z_pos_far  = oz + dz * far_t;
        active &= solution_found && !no_hit && !out_bounds && !in_bounds &&
                  (z_pos_near >= Value(0.f) || z_pos_far >= Value(0.f));    // 追加

        FloatP t = dr::select(
            active, dr::select(near_t < Value(0.0) || z_pos_near < Value(0.0), FloatP(far_t), FloatP(near_t)),    // 変更
            dr::Infinity<FloatP>);
        /* 変更ここまで */
    ...
    dr::mask_t<FloatP> ray_test_impl(const Ray3fP &ray,
        ...
        return solution_found && !out_bounds && !in_bounds && active && (z_pos_near >= Value(0.f) || z_pos_far >= Value(0.f));    // 最後の条件を追加
    }
    ...
    void optix_prepare_geometry() override {
        if constexpr (dr::is_cuda_v<Float>) {
            if (!m_optix_data_ptr)
                m_optix_data_ptr = jit_malloc(AllocType::Device, sizeof(OptixMyHalfSphereData));

            OptixMyHalfSphereData data = { bbox(), m_to_object.scalar(),
                                     (Vector<float, 3>) m_center.scalar(),
                                     (float) m_radius.scalar() };

            jit_memcpy(JitBackend::CUDA, m_optix_data_ptr, &data,
                       sizeof(OptixMyHalfSphereData));
        }
    }
#endif
...
MI_IMPLEMENT_CLASS_VARIANT(MyHalfSphere, Shape)
MI_EXPORT_PLUGIN(MyHalfSphere, "MyHalfSphere intersection primitive");
