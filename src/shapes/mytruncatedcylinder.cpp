...
#include <drjit/packet.h>
    ...
    #include "optix/mytruncatedcylinder.cuh"
#endif
...
class MyTruncatedCylinder final : public Shape<Float, Spectrum> {
public:
    ...
    MyTruncatedCylinder(const Properties &props) : Base(props) {
    ...
    ray_intersect_preliminary_impl(const Ray3fP &ray_,
        ...
        //std::cout << "ray_intersect_preliminary_impl called" << std::endl;
        ...
        /* 以下追加・変更 */
        // 注：length=1.0, radius=1.0 が前提となっているプログラムである点に注意
        Value x_pos_near = ox + dx * near_t,
              x_pos_far  = ox + dx * far_t;
        
        // 円柱左下を原点、円柱の直径を1とした座標系における、レイとの交点のx座標を計算
        Value x_new_near = (x_pos_near + Value(1.0)) * Value(0.5),
              x_new_far  = (x_pos_far  + Value(1.0)) * Value(0.5);

        // 上記座標系だと、円柱断面の式はz=xで表されるから、レイとの交点のz座標がx以下（すなわち断面のz座標以下）であれば、斜切り円柱とレイは接触していると判断できる（第4条件を追加）
        active &= solution_found && !out_bounds && !in_bounds &&
                  ((z_pos_near >= Value(0.0) && z_pos_near <= length && near_t >= Value(0.0) && z_pos_near <= x_new_near) ||
                   (z_pos_far  >= Value(0.0) && z_pos_far <= length  && far_t <= maxt && z_pos_far <= x_new_far));
        /* 変更ここまで */
    ...
    dr::mask_t<FloatP> ray_test_impl(const Ray3fP &ray_,
        ...
        //std::cout << "ray_test_impl called" << std::endl;
    ...
    void optix_prepare_geometry() override {
        if constexpr (dr::is_cuda_v<Float>) {
            if (!m_optix_data_ptr)
                m_optix_data_ptr = jit_malloc(AllocType::Device, sizeof(OptixTCylinderData));

            OptixTCylinderData data = { bbox(), m_to_object.scalar(),
                                       (float) 1.f,
                                       (float) 1.f };

            jit_memcpy(JitBackend::CUDA, m_optix_data_ptr, &data, sizeof(OptixTCylinderData));
        }
    }
#endif
    ...
    std::string to_string() const override {
        ...
        oss << "MyTruncatedCylinder[" << std::endl
...
MI_IMPLEMENT_CLASS_VARIANT(MyTruncatedCylinder, Shape)
MI_EXPORT_PLUGIN(MyTruncatedCylinder, "MyTruncatedCylinder intersection primitive");
