...
#ifdef __CUDACC__
extern "C" __global__ void __intersection__myhalfcylinder() {
    const OptixHitGroupData *sbt_data = (OptixHitGroupData*) optixGetSbtDataPointer();
    OptixHalfCylinderData *cylinder = (OptixHalfCylinderData *)sbt_data->data;
    ...
    bool valid_intersection =
        ...
        ((z_pos_near >= 0.f && z_pos_near <= cylinder->length && near_t >= 0.f && x_pos_near >= 0.0f) ||
         (z_pos_far  >= 0.f && z_pos_far  <= cylinder->length && far_t <= ray.maxt && x_pos_far >= 0.0f));

    float t = (z_pos_near >= 0 && z_pos_near <= cylinder->length && near_t >= 0.f && x_pos_near >= 0.0f ? near_t : far_t);
...
extern "C" __global__ void __closesthit__myhalfcylinder() {
    const OptixHitGroupData *sbt_data = (OptixHitGroupData *) optixGetSbtDataPointer();
