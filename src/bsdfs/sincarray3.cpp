...
class sincarray3 final : public BSDF<Float, Spectrum>
{
...
public:
    ...
    sincarray3(const Properties &props) : Base(props)
    {
    ...
    void make_sincarray1_multi(float a, int expo)
    {
        ...
        float CornerSize = 0.080f;
        for (int i = 0; i < N; ++i)
        {
            ScalarFloat sincvalue = ScalarFloat(0.0f);
            // int count = 600*2*tan(M_PI / a) / CornerSize;
            int count = 300;
            for (int n = -count/2; n <= count/2; ++n) {
                ScalarFloat arg = a * (dr::Pi<Float>*i / (2.0f*N)) - n*CornerSize;
    ...
    void traverse(TraversalCallback *callback) override
    {
        ...
        // M = 1440; N = 360;
        // make_sincarray3(m_a.get()->max(), 2);

        // make_sincarray1(m_a.get()->max(), 2);
        ...
        // ) const
    {
        ...
        // Float del_theta = point*dr::Pi<Float>/(a*10.0f);
        ...
        // UnpolarizedSpectrum sincvalue = ScalarFloat(0.0);
        // for (int n = -50; n = 50; ++n) {
        //     UnpolarizedSpectrum arg = m_a->eval(si, active)*(del_theta - 0.08*n);
        //     sincvalue += dr::pow(dr::sin(arg), 2) / dr::pow(arg, 2);
        //     sincvalue = dr::select(
        //         dr::isfinite(sincvalue),
        //         sincvalue,
        //         ScalarFloat(1.0)
        ...
        //     ScalarFloat(1.0)
        // );
        UnpolarizedSpectrum sincvalue = ScalarFloat(1.0);
    ...
    Spectrum eval(
        ...
        // UnpolarizedSpectrum value = m_reflectance->eval(si, active) * sincvalue;
        UnpolarizedSpectrum value = m_reflectance->eval(si, active) * ScalarFloat(1.0);
    ...
    std::pair<Spectrum, Float> eval_pdf(
        ...
        const Vector3f &wo, Mask active
            ...
            UnpolarizedSpectrum value = m_reflectance->eval(si, active)*ScalarFloat(1.0f);
            Float pdf = dr::select(cos_theta_i > 0.f && cos_theta_o > 0.f, 1.0f, 0.f);
...
MI_IMPLEMENT_CLASS_VARIANT(sincarray3, BSDF)
MI_EXPORT_PLUGIN(sincarray3, "sincarray3")
