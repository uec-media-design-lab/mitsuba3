...
class RoughRetroreflectorCP final : public BSDF<Float, Spectrum> {
public:
    MI_IMPORT_BASE(BSDF, m_flags, m_components)
    MI_IMPORT_TYPES(Texture, MicrofacetDistribution)

    RoughRetroreflectorCP(const Properties &props) : Base(props) {
        ...
        // read LUT
        LUTDirectory = "/home/sugawara.ryo/makeLUT/2024.06.02diff/LUTs/"
        LUTFileName = "makeLUTtest0.0001.json"
        // std::ifstream file("RRshiftTensor.json");
        std::ifstream file(LUTDirectory+LUTFileName);
        if (!file.is_open()) {
        ...
        for (const auto& dim1 : jsonData) {
            ...
            for (const auto& dim2Data : dim1) {
                ...
                for (const auto& dim3Data : dim2Data) {
                    ...
                    if (i == 0) {
                        std::cout << "phi = " << dim3Data << std::endl;
                    } else {
    ...
    void traverse(TraversalCallback *callback) override {
        ...
        // callback->put_parameter("corner_size", cornerCubeSize);
    ...
    std::string to_string() const override {
        ...
        oss << "RoughRetroeflectorCP[" << std::endl
...
MI_IMPLEMENT_CLASS_VARIANT(RoughRetroreflectorCP, BSDF)
MI_EXPORT_PLUGIN(RoughRetroreflectorCP, "Rough retroreflector copied(test)")
